  ...
  <!-- Trigger the modal with a button -->
		...
		<table id = "table" class="table table-bordered">
			...
			<tbody>
				...
				$query4 = mysqli_query($conn, "SELECT * FROM `coordiantor` as coo join coordiantor_assigndep as cooassign on coo.ccode=cooassign.ccode where cooassign.depcode='".$fetch['depcode']."'") or die(mysqli_error());
				$fetch5 = mysqli_fetch_array($query4);

					$query42 = mysqli_query($conn, "SELECT * FROM `departments` where depcode='".$fetch['depcode']."'") or die(mysqli_error());
				$fetch52 = mysqli_fetch_array($query42)
				   ...
				   ?>
					<tr>
						<td><?php echo $fetch5['ccode']?></td>
						<td><?php echo $fetch5['depcode']?></td>
						<td><?php echo $fetch52['depname']?></td>
