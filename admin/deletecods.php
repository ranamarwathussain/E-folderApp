<?php
	...
	if(ISSET($_GET['id'])){
		...
		if($_GET['assign']==1)
		{
				mysqli_query($conn, "delete from cod_assigndep WHERE `aid` = '$did'") or die(mysqli_error());
				echo "<script>alert('Successfully Deleted Assigned Department!')</script>";
		echo "<script>window.location = 'cods.php'</script>";
		}
		else
		{
		mysqli_query($conn, "delete from cods WHERE `cocode` = '$did'") or die(mysqli_error());
		mysqli_query($conn, "delete from cod_assigndep WHERE `cocode` = '$did'") or die(mysqli_error());
