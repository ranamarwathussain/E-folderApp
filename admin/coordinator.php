...
<body>
  ...
  <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal"><span class="glyphicon glyphicon-plus"></span> Add Coordinator</button>
...
<br /><br />
		...
		<table id = "table" class="table table-bordered">
			...
			<tbody>
				<?php
					$query = mysqli_query($conn, "SELECT * FROM `coordiantor`") or die(mysqli_error());
					while($fetch = mysqli_fetch_array($query)){
				?>
					<tr class="del_student<?php echo $fetch['cid']?>">
						<td><?php echo $fetch['ccode']?></td>
						...
						<a href="deletecordinator.php?id=<?php echo $fetch['ccode']?>" ><button class="btn btn-danger btn-delete"  type="button"><span class="glyphicon glyphicon-trash"></span> Delete</button></center></a></td>
					</tr>
	<div class="modal fade" id="edit_modal<?php echo $fetch['cid']?>" aria-hidden="true">
					...
					<div class="modal-body">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="number" name="ccode" value="<?php echo $fetch['ccode']?>" class="form-control" disabled/>
							</div>
							<div class="form-group">
		...
		<br /><br />
			<b><h2>All Coordinator Assignments</h2></b>
		<table id = "table" class="table table-bordered">
			<thead>
				<tr>
					<th>Emp Code</th>
			...
			<tbody>
				<?php
					$query = mysqli_query($conn, "SELECT * FROM `coordiantor` as co JOIN coordiantor_assigndep as cass WHERE cass.ccode=co.ccode;") or die(mysqli_error());
					while($fetch = mysqli_fetch_array($query)){
				...
				?>
					<tr class="del_student<?php echo $fetch['cid']?>">
						...
						<td><center> 
						<a href="deletecordinator.php?assign=1&id=<?php echo $fetch['aid']?>" ><button class="btn btn-danger btn-delete"  type="button"><span class="glyphicon glyphicon-trash"></span> Delete</button></center></a></td>
					</tr>
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<form method="POST" action="save_coordinator.php">
								<label>Emp Code</label>
								 
								<label class="form-control-label">Select Coodinator<span class="text-danger ml-2"> </span></label>
                         <?php
                        $qry= "SELECT * FROM coordiantor ORDER BY cid ASC";
                        ...
                        if ($num > 0){
                          ...
                          echo'<option value="'.$rows['ccode'].'" >'.$rows['ccode'].'-'.$rows['FirstName'].'-'.$rows['LastName'].'</option>';
                              }
                                 echo '</select>';
                            }
                           ?>
                        ...
                        if ($num > 0){
                          ...
                          echo'<option value="">--Select Item Code--</option>';
                          while ($rows = $result->fetch_assoc()){
                          echo'<option value="'.$rows['depcode'].'" >'.$rows['depcode'].'-'.$rows['depname'].'-'.$rows['depschool'].'</option>';
                              }
                                 echo '</select>';
                            }
                           ?>  
