...
<?php
	...
	if(ISSET($_GET['id'])){
		...
		mysqli_query($conn, "delete from faculty WHERE `fcode` = '$did'") or die(mysqli_error());
        //mysqli_query($conn, "delete from deans_assigndep WHERE `dcode` = '$did'") or die(mysqli_error());
        mysqli_query($conn, "delete from  courses_assign WHERE `fcode` = '$did'") or die(mysqli_error());
